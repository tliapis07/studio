...
export default function LoginPage() {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      if (result.user) {
        console.log("[Auth] Success: Signed in as", result.user.displayName);
        toast({ title: "Welcome back", description: `Signed in as ${result.user.displayName}` });
        router.push('/dashboard');
      }
    } catch (err: any) {
      setIsLoading(false);
      console.warn("[Auth] Interaction result:", err.code);
      
      // Specific user-friendly handling for common Firebase Auth popup errors
      if (err.code === 'auth/popup-closed-by-user') {
        toast({ 
          title: "Sign-in Cancelled", 
          description: "The window was closed before completion. Try again?",
          variant: "default"
        });
        return;
      }
      ...
      if (err.code === 'auth/popup-blocked') {
        toast({ 
          variant: "destructive",
          title: "Popup Blocked", 
          description: "Please allow popups for this site in your browser settings and retry." 
        });
        return;
      }
      ...
      if (err.code === 'auth/network-request-failed') {
        toast({ 
          ...
          description: "Network request failed. Please check your internet connection." 
        });
        return;
      }
  ...
  // Global loading overlay during verification or manual sign-in process
      ...
      <div className="min-h-screen flex flex-col items-center justify-center bg-background dark p-8">
        <div className="flex flex-col items-center gap-6 max-w-sm text-center">
          ...
          <div className="space-y-2">
            <h2 className="text-xl font-black font-headline tracking-tight uppercase">Verifying Identity</h2>
            <p className="text-xs font-black uppercase tracking-widest text-muted-foreground animate-pulse">Establishing Secure Partner Session...</p>
          </div>
        </div>
      </div>
    );
          ...
          <Card className="border-border/50 bg-card/50 backdrop-blur-xl shadow-2xl rounded-3xl overflow-hidden border-2">
            ...
            <CardContent className="grid gap-6 px-10 pb-10">
              <div className="space-y-3 text-center">
                ...
                <div className="flex items-center justify-center gap-2 text-[10px] text-muted-foreground font-black uppercase tracking-widest mt-2">
                  <AlertCircle className="h-3 w-3" />
                  <span>Allow popups if blocked</span>
                </div>
              </div>
              <div className="relative">
