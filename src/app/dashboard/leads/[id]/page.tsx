...
export default function LeadDetailPage() {
  ...
  const leadRef = useDoc<Lead>(doc(db, 'leads', params.id as string));
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1">
          ...
          <div className="flex gap-4 mt-1">
            ...
            <span className="text-muted-foreground text-xs font-bold flex items-center gap-1.5 uppercase tracking-wider"><Calendar className="h-3 w-3" /> {format(lead.createdAt?.toDate ? lead.createdAt.toDate() : new Date(lead.createdAt), 'PP')}</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                {lead.nextFollowUpAt ? (
                  <div className="flex items-center justify-between p-3 bg-background rounded-xl border border-primary/20">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-sm font-black">{format(lead.nextFollowUpAt.toDate(), 'PPP p')}</span>
                    </div>
            ...
            <Card className="bg-accent/5 border-accent/20 border-2">
              ...
              <CardContent>
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-sm font-black">Partner Organization</p>
        ...
        <div className="space-y-6">
          <Card className="bg-primary/5 border-2 border-primary/20 shadow-xl rounded-2xl overflow-hidden">
            <CardHeader className="bg-primary/10 border-b border-primary/10">
              <CardTitle className="text-[11px] font-black uppercase tracking-[0.2em] flex items-center gap-2">
                <Sparkles className="h-4 w-4" /> Lead Score
              </CardTitle>
            </CardHeader>
            <CardContent className="p-6">
              <div className="flex items-end gap-2 mb-4">
                <span className="text-5xl font-black text-primary font-headline leading-none">88</span>
